...
public class PowerOffAlarmUtility {
            ...
            public void onReceive(Context context, Intent intent) {
                ...
                if ("android.intent.action.normal.boot".equals(action)) {
                    ...
                    ActivityManagerPlus.removeIPOWin();
                        ...
                        isAlarmBoot())
                {
                        Slog.v(TAG, "power off alarm enabled with IPO boot");
                ...
                } else if (ACTION_AIRPLANE_CHANGE_DONE.equals(action)) {
                    ...
                    Log.d(TAG, "onReceive, ACTION_AIRPLANE_CHANGE_DONE = " + airplaneMode
                         + " , mIsAirModeEnableBeforeAlarmBoot = " + mIsAirModeEnableBeforeAlarmBoot
                         + " , mNeedTurnOffAirMode = " + mNeedTurnOffAirMode);
                    if (!mIsAirModeEnableBeforeAlarmBoot && mNeedTurnOffAirMode) {
                        mNeedTurnOffAirMode = false;
                        SystemProperties.set("persist.sys.ams.recover", "false");
                        Settings.Global.putInt(mContext.getContentResolver(),
                            Settings.Global.AIRPLANE_MODE_ON, 0);
                        Intent intent_chg_done = new Intent(Intent.ACTION_AIRPLANE_MODE_CHANGED);
                        intent_chg_done.addFlags(Intent.FLAG_RECEIVER_REPLACE_PENDING);
                        intent_chg_done.putExtra("state", false);
                        mContext.sendBroadcast(intent_chg_done);
                        Log.v(TAG, "turn off flight mode with airplane mode change done");
                    }
                }
            }
        }, filter);
    ...
    private void checkFlightMode(boolean recover, boolean shutdown) {
        ...
        if (mRollback) {
            ...
            if (!shutdown) {
                if (isAirplanemodeAvailableNow() && mNeedTurnOffAirMode) {
                    mNeedTurnOffAirMode = false;
                    Intent intent_avail = new Intent(Intent.ACTION_AIRPLANE_MODE_CHANGED);
                    intent_avail.addFlags(Intent.FLAG_RECEIVER_REPLACE_PENDING);
                    intent_avail.putExtra("state", false);
                    mContext.sendBroadcast(intent_avail);
            ...
            if (!mode) {
                ...
                mIsAirModeEnableBeforeAlarmBoot = false;
                ...
                mIsAirModeEnableBeforeAlarmBoot = true;
            }
        }
    }
