    ...
    class AgpsNetReq {
        ...
        void getDefaultNet() {
            ...
            if (null != net) {
                ...
                if (null != netCap && netCap.hasTransport(NetworkCapabilities.TRANSPORT_WIFI)) {
                    if (DEBUG) log("to use NetWiFi");
                    mRouteType = ConnectivityManager.TYPE_WIFI;
                } else if (null != netCap && netCap.hasTransport(NetworkCapabilities.TRANSPORT_VPN)) {
                    if (DEBUG) log("to use NetVPN");
                    mRouteType = ConnectivityManager.TYPE_VPN;
                } else {
        ...
        void requestRoute(InetAddress ia) {
            if (ConnectivityManager.TYPE_NONE != mRouteType) {
                ...
                if (!result) {
                    ...
                    if (mRouteType == ConnectivityManager.TYPE_MOBILE_SUPL) {
                        result = mConnManager.requestRouteToHostAddress(
                                ConnectivityManager.TYPE_MOBILE, ia);
                        if (!result) {
                            log("Error requesting route (TYPE_MOBILE) to host: " + ia);
                        } else if (DEBUG) {
                            log("Requesting route (TYPE_MOBILE) to host: " + ia);
                        }
                    }
                } else if (DEBUG) {
