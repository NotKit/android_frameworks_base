...
public final class AMEventHook {
    ...
    private void showStartProcessForActivity(AMEventHookData.StartProcessForActivity data) {
        ...
        String packageName =
                data.getString(AMEventHookData.StartProcessForActivity.Index.packageName);
    ...
    boolean isDebuggableMessage(Event event) {
        ...
        case AM_ActivityThreadResumedDone:
            return false;
        case AM_StartProcessForActivity:
            return false;
        default:
            return true;
        }
    ...
    private AMEventHookResult onEvent(Event event, Object data) {
        ...
        case AM_ActivityThreadResumedDone:
            result = onActivityThreadResumedDone(
                (AMEventHookData.ActivityThreadResumedDone) data);
            break;
        case AM_SystemUserUnlock:
            result = onSystemUserUnlock(
                (AMEventHookData.SystemUserUnlock) data);
            break;
        case AM_StartProcessForActivity:
            result = onStartProcessForActivity(
                (AMEventHookData.StartProcessForActivity) data);
            break;
        default:
     ...
     */
    private AMEventHookResult onUpdateSleep (
        AMEventHookData.UpdateSleep data) {
        AMEventHookResult result = null;
    ...
    /**
     * Handle AM_SystemUserUnlock event.
     */
    private AMEventHookResult onSystemUserUnlock(
        AMEventHookData.SystemUserUnlock data) {
        if (AMEventHook.DEBUG_EVENT_DETAIL) {
        ...
        /// M: add for running booster feature @{
        if (null != runningBoosterService) {
            runningBoosterService.onSystemUserUnlock(data);
        }
        /// M: add for running booster feature @}
    ...
    /**
     * Handle AM_StartProcessForActivity event.
     */
    private AMEventHookResult onStartProcessForActivity(
        AMEventHookData.StartProcessForActivity data) {
        if (AMEventHook.DEBUG_EVENT_DETAIL) {
        ...
        /// M: PerfService @{
        if(mPerfService != null) {
            mPerfService.amsBoostProcessCreate(data);
        }
        /// M: PerfService @}
        return result;
    }
