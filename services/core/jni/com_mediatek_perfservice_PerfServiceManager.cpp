...
static void init()
{
    ...
    func = dlsym(handle, "perfSetExclusiveCore");
...
static int
android_server_PerfSetExclusiveCore(JNIEnv *env, jobject thiz, jint pid, jint cpuMask)
{
#if defined(MTK_PERFSERVICE_SUPPORT)
    if (!inited)
        init();
    ...
    if (perfSetExclusiveCore) {
        return perfSetExclusiveCore(pid, cpuMask);
    }

    ALOGE("perfSetExclusiveCore bypassed!");
#endif
    return -1;
}
...
static JNINativeMethod sMethods[] = {
    ...
    {"nativePerfSetExclusiveCore", "(II)I", (int *)android_server_PerfSetExclusiveCore},
};
