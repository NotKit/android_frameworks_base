...
public interface RILConstants {
    ...
    */
    int PREFERRED_NETWORK_MODE = SystemProperties.get("ro.boot.opt_c2k_support").equals("1") ?
...
*/
    ...
    int RIL_UNSOL_DATA_ATTACH_APN_CHANGED = (RIL_UNSOL_VENDOR_BASE + 95);

    /// M: IMS Provisioning URC. @{
    int RIL_UNSOL_GET_PROVISION_DONE = (RIL_UNSOL_VENDOR_BASE + 96);
    /// @}
    /// M: To trigger OTASP @{
    int RIL_UNSOL_TRIGGER_OTASP = (RIL_UNSOL_VENDOR_BASE + 97);
    /// @}
    /// M: CC: Vzw ECC/hVoLTE redial
    int RIL_UNSOL_CALL_REDIAL_STATE = (RIL_UNSOL_VENDOR_BASE + 98);

    // MTK-START, SMS part
    // CDMA Card inital ESN or MEID for CT self register
    int RIL_UNSOL_CDMA_CARD_INITIAL_ESN_OR_MEID = (RIL_UNSOL_VENDOR_BASE + 99);
    // MTK-END, SMS part

    // M: IMS ECT @{
    int RIL_UNSOL_ECT_INDICATION = (RIL_UNSOL_VENDOR_BASE + 100);
    /// @}

    /// M: IMS RTP Report @{
    int RIL_UNSOL_IMS_RTP_INFO = (RIL_UNSOL_VENDOR_BASE + 101);
    /// @}

    // APC
    int RIL_UNSOL_PSEUDO_BS_INFO_LIST = (RIL_UNSOL_VENDOR_BASE + 102);

    /// M: Sync volte setting value. @{
    int RIL_UNSOL_VOLTE_SETTING = (RIL_UNSOL_VENDOR_BASE + 103);
    /// @}
}
