...
public class PerfServiceWrapper implements IPerfServiceWrapper {
    ...
    private static final int AMS_BOOST_TIME = 4000;
    ...
    public int getPackAttr(String packName, int cmd) {
        ...
        try {
            init();
            if(sService != null)
                sService.getPackAttr(packName, cmd);
        } catch (RemoteException e) {
    ...
    public void amsBoostResume(AMEventHookData.BeforeActivitySwitch data) {
        ...
        if (lastPkg == null || !lastPkg.equalsIgnoreCase(nextPkg)) {
    ...
    public void amsBoostProcessCreate(AMEventHookData.StartProcessForActivity data) {
        ...
        if(reason.compareTo("activity") == 0) {
            ...
            //appBoostEnable(data.getString(StartProcessForActivity.Index.packageName));
        }
    }
