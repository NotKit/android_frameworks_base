...
public class DcfDecoder {
    ...
    static {
        sIsOmaDrmEnabled = SystemProperties.getBoolean("ro.mtk_oma_drm_support", false);
        if (sIsOmaDrmEnabled) {
    ...
    private static native byte[] decryptDcfFile(FileDescriptor fd, int size, int action);
    ...
    private static Bitmap decodeDrmImage(FileDescriptor fd, int size, Options opts) {
        ...
        byte[] clearData = decryptDcfFile(fd, size, action);
        if (clearData == null) {
            return null;
        }
